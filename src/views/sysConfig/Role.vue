<template>
  <PageWrapper>
    <template #headerContent>
      <Form layout="horizontal" :model="params" ref="headerFormRef">
        <Row :gutter="32">
          <Col :span="7">
            <FormItem :label="t('routes.sysConfig.role.roleName')">
              <Input v-model:value="params.roleName" />
            </FormItem>
          </Col>
          <Col :span="7">
            <FormItem :label="t('routes.sysConfig.role.roleState')">
              <Select v-model:value="params.enable" :options="enableList" :allowClear="true" />
            </FormItem>
          </Col>
          <Col :span="7">
            <FormItem :label="t('common.createDate')">
              <DatePicker.RangePicker
                v-model:value="params.createTime"
                :valueFormat="dateValueFormat"
              />
            </FormItem>
          </Col>
        </Row>
        <Row :gutter="32">
          <Col :span="7">
            <FormItem :label="t('common.creator')">
              <Input v-model:value="params.createUserName" />
            </FormItem>
          </Col>
          <Col :span="7">
            <FormItem :label="t('common.updater')">
              <Input v-model:value="params.editUserName" />
            </FormItem>
          </Col>
          <Col :span="7">
            <FormItem :label="t('common.updateDate')">
              <DatePicker.RangePicker
                v-model:value="params.editTime"
                :valueFormat="dateValueFormat"
              />
            </FormItem>
          </Col>
          <Col :span="3">
            <Button type="primary" @click="getRecordList">{{ t('common.queryText') }}</Button>
            <Button type="primary" @click="handleResetClick">{{ t('common.resetText') }}</Button>
          </Col>
        </Row>
        <Row>
          <Col :span="10">
            <Button type="primary" @click="handleCreateClick" v-if="pageFn.add">
              <template #icon><PlusOutlined /></template>
              {{ t('common.create', { name: t('routes.sysConfig.role.role') }) }}</Button
            >
          </Col>
        </Row>
      </Form>
    </template>
    <VTableVue
      :total="total"
      :columns="columns"
      :dataSource="records"
      :actionColumnWidth="250"
      :rowSelection="rowSelection"
      :headerPagination="false"
      rowKey="transactionId"
      @page-change="onPageChange"
    >
      <template #actionBodyColumn="{ record }" v-if="pageFn.edit || pageFn.disable">
        <Button
          class="action-btn"
          type="link"
          @click="handleEditClick(record)"
          v-if="pageFn.edit"
          >{{ t('common.edit') }}</Button
        >
        <Button
          class="action-btn"
          type="link"
          @click="handleToggleUserState(record)"
          v-if="pageFn.disable"
          >{{ record.enable ? t('common.disableText') : t('common.enableText') }}</Button
        >
      </template>
      <template #bodyColumn="{ column, text }">
        <!-- <template v-if="column.key === 'deadlineDate' || column.key === 'takeEffectDate'">
          {{ formatDate(text) }}
        </template> -->
        <template v-if="column.dataIndex === 'enable'">
          {{ text ? t('common.enableText') : t('common.disableText') }}
        </template>
        <template v-else-if="column.dataIndex === 'relateUserCount'">
          <Button type="link">{{ text }}</Button>
        </template>
      </template>
    </VTableVue>
    <NewRoleModal
      v-if="modalVisible"
      :is="modalVisible"
      :role="visibleModalRole"
      @hide="modalVisible = false"
      @reload="getRecords()"
    />
    <!-- <RecordNoteModal :id="noteModalId" v-model:visible="noteModalVisible" /> -->
  </PageWrapper>
</template>
<script lang="ts" setup>
  import { Form, FormItem, Row, Col, Input, Button, Select, DatePicker } from 'ant-design-vue';
  import type { FormInstance } from 'ant-design-vue';
  import { PlusOutlined } from '@ant-design/icons-vue';
  import { ref, shallowRef, computed, Ref } from 'vue';
  import { PageWrapper } from '/@/components/Page';
  import { getPagedRoleList, changeUserState } from '/@/api/sysConfig/role.api';
  import { createUseRecords } from '/@/utils/my/useRecords';
  import VTableVue, { VTableColumnType } from '../trade/VTable.vue';
  import NewRoleModal from './components/NewRoleModal.vue';
  // import LoginLogModal from './components/LoginLogModal.vue';
  import { useI18n } from '/@/hooks/web/useI18n';
  import usePageFn from '/@/utils/my/usePageFn';
  const pageFn = usePageFn();
  const { t } = useI18n();

  // const RadioButton = Radio.Button;
  const dateValueFormat = 'YYYY-MM-DD';
  // const tradeType = ref('');
  const enableList = computed(() => [
    {
      label: t('common.enableText'),
      value: 1,
    },
    {
      label: t('common.disableText'),
      value: 0,
    },
  ]);
  const columns = computed<VTableColumnType[]>(() => [
    {
      title: 'ID',
      dataIndex: 'roleId',
      visible: true,
    },
    {
      title: t('routes.sysConfig.role.roleName'),
      dataIndex: 'roleName',
      visible: true,
    },
    {
      title: t('routes.sysConfig.role.relateUserCount'),
      dataIndex: 'userCount',
      visible: true,
    },
    {
      title: t('common.creator'),
      dataIndex: 'createUserName',
      visible: true,
    },
    {
      title: t('common.createTime'),
      dataIndex: 'createTime',
      visible: true,
    },
    {
      title: t('common.updater'),
      dataIndex: 'editeUserName',
      visible: true,
      // width: 150,
    },
    {
      title: t('common.updateTime'),
      dataIndex: 'editTime',
      visible: true,
    },
    {
      title: t('common.state'),
      dataIndex: 'enable',
      visible: true,
    },
  ]);

  const { total, params, records, getRecords, rowSelection } =
    createUseRecords<RoleItem[]>()(getPagedRoleList);

  getRecords();
  const getRecordList = () => {
    getRecords();
  };

  const onPageChange = (page: number, pageSize: number) => {
    params.value.page = page;
    params.value.limit = pageSize;
    getRecords();
  };

  const headerFormRef = ref<FormInstance>() as Ref<FormInstance>;
  const handleResetClick = () => {
    params.value = {};
    headerFormRef.value.resetFields();
  };
  const handleToggleUserState = (user: RoleItem) => {
    changeUserState(user.roleId, 1 - +user.enable).then(getRecords);
  };
  const modalVisible = shallowRef<boolean>(false);
  const visibleModalRole = ref<RoleItem>();
  const handleEditClick = (user: RoleItem) => {
    modalVisible.value = true;
    visibleModalRole.value = user;
  };
  const handleCreateClick = () => {
    modalVisible.value = true;
    visibleModalRole.value = void 0;
  };
</script>
