    ...
    <template #headerContent>
      <Form layout="horizontal" :model="params">
        <Row :gutter="32">
          ...
          <Col :span="8">
            <FormItem :label="t('routes.sysConfig.lawyer.lawyerName')">
        ...
        <Row :gutter="32">
          ...
          <Col :span="8">
            ...
            <Button type="primary" @click="handleCreateClick" v-if="pageFn.add">
              {{ t('common.create', { name: t('routes.sysConfig.lawyer.lawyer') }) }}</Button
            >
          </Col>
        </Row>
      </Form>
...
<script lang="ts" setup>
  ...
  import { getPagedLawyerList } from '/@/api/sysConfig/lawyer.api';
  ...
  const { total, params, records, getRecords, rowSelection } =
    createUseRecords<LawyerInfoItem[]>()(getPagedLawyerList);
  ...
  const visibleModalCustomer = ref<LawyerInfoItem>();
  ...
  const handleShowTradeCount = (row: LawyerInfoItem) => {
